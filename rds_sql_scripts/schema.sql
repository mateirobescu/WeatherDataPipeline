...
CREATE TABLE IF NOT EXISTS countries (
    ...
    CONSTRAINT chk_iso3_code_len CHECK ( CHAR_LENGTH(iso3_code) = 3)
);
...
CREATE TABLE IF NOT EXISTS cities (
    ...
    CONSTRAINT chk_city_id_len CHECK ( CHAR_LENGTH(id) <= 10  )
);
...
CREATE TABLE IF NOT EXISTS weather_readings (
    ...
    CONSTRAINT chk_weather_cityid_len CHECK ( CHAR_LENGTH(city_id) <= 10 )
);